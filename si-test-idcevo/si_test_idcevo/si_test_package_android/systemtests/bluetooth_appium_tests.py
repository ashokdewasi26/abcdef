...
class TestBluetoothAppium:
    ...
    def test_01_turn_on_bluetooth_via_cardomain_ui(self):
        """
        Turn ON the bluetooth via appium
        """
        Connectivity.turn_on_bluetooth_ui()

        bluetooth_state = Connectivity.check_bluetooth_state_ui()
        utils.get_screenshot_and_dump(self.test, self.test.results_dir, "turn_on_bluetooth_via_ui")
        assert_true(bluetooth_state, "Unable to turn on bluetooth")
    ...
    def test_02_turn_off_bluetooth_via_cardomain_ui(self):
        """
        Turn OFF the bluetooth via appium
        """
        Connectivity.turn_off_bluetooth_ui()

        bluetooth_state = Connectivity.check_bluetooth_state_ui()
        utils.get_screenshot_and_dump(self.test, self.test.results_dir, "turn_off_bluetooth_via_ui")
        assert_false(bluetooth_state, "Unable to turn off bluetooth")
