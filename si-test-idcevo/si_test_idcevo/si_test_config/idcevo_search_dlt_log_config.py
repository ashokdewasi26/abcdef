...
"""
IDCEVO Config file to define the DLT messages which will be searched in DLT log, by 'search_logs_tests.py' post test.
All DLT 'apid' and 'ctid' listed bellow, must also be defined in 'dlt_list_of_interest_idcevo.csv' plugin.
"""
...
DLT_LOG_VERIFICATION = {
    "search_for_acl_plugin_initialization_logs": {
        ...
        "pattern": [
            [
                ...
                r"Client 0x6310 Loading plug-in library: libvsomeip-netsec-plugin.so.1 succeeded!",
                r"OFFER\(6310\): \[fee2..*:\d.\d\] \(true\)",
                r"REQUEST\(6310\): \[f8a0..*\]",
    ...
    "search_for_testability_logs": {
        ...
        "pattern": [
            [
                r"REGISTER EVENT\(.*\): \[0101.0063.8001:eventtype=0:is_provided=true:reliable=2\]",
                r"REGISTER EVENT\(.*\): \[0101.0063.8002:eventtype=0:is_provided=true:reliable=2\]",
                r"REGISTER EVENT\(.*\): \[0101.0063.8003:eventtype=0:is_provided=true:reliable=1\]",
                r"REGISTER EVENT\(.*\): \[0101.0063.8004:eventtype=0:is_provided=true:reliable=2\]",
                r"REGISTER EVENT\(.*\): \[0101.0063.8005:eventtype=2:is_provided=true:reliable=2\]",
                r"REGISTER EVENT\(.*\): \[0101.0063.8006:eventtype=2:is_provided=true:reliable=2\]",
                r"REGISTER EVENT\(.*\): \[0101.0063.8007:eventtype=2:is_provided=true:reliable=2\]",
                r"REGISTER EVENT\(.*\): \[0101.0063.8008:eventtype=2:is_provided=true:reliable=1\]",
                r"REGISTER EVENT\(.*\): \[0101.0063.800b:eventtype=0:is_provided=true:reliable=2\]",
            ]
        ],
        "domain": "Network",
        "docstring": """[SIT_Automated] SOME/IP - Testability - EnhancedTestabilityServiceHigh2 is offered""",
        "feature": ["TESTABILITY"],
        "duplicates": "IDCEVODEV-11525",
    },
    ...
    "search_for_someip_logs": {
        ...
        "pattern": [
            [
                ...
                r".*Application\(vsomeipd, 6310\) is initialized \(11, 100\).*",
                r".*Client 0x6310 Loading plug-in library: libvsomeip-heartbeat-plugin.so.1 succeeded!.*",
                r".*vsomeipd sent READY to systemd watchdog.*",
                r".*vsomeipd systemd watchdog is enabled.*",
                r".*Starting vsomeip application \"vsomeipd\" \(6310\) using 4 threads I/O nice -5.*",
                r".*Client \[6310\] routes unicast:\d+\.\d+(\.\d+)*, netmask:\d+\.\d+(\.\d+)*",
                ...
                r"Client \[6310\] @ \d+\.\d+(\.\d+)*:\d+ is connecting to \[\w+\] @ \d+\.\d+(\.\d+)*",
                r".*REGISTERED_ACK\(\w+\).*",
                r".*OFFER\(\w+\): \[.*\] \(true\).*",
    ...
    "search_for_soc_dlt_sfi_logs": {
        "apid": ["LOGM"],
        "ctid": ["INTE"],
        "pattern": [
            [
                r".*DLT_BRIDGE: /usr/lib/libSFILogReader.so.1.0.0 Plugin loaded successfully.*",
                r".*SFIReader_Plugin: SFI_LR constructor being initialized!.*",
            ],
        ],
        "domain": "RTOS",
        "docstring": """[SIT_Automated] SFI DLT_Bridge init success""",
        "feature": ["INTER_NODE_COMMUNICATION_SFI_LINUX_COMMUNICATION"],
        "duplicates": "IDCEVODEV-82856",
    },
}
