# Copyright (C) 2025. CTW PT. All rights reserved.
import configparser
import csv
import logging
import os
import re
import shutil
import tempfile
from pathlib import Path
from unittest import skipIf

from mtee.testing.support.target_share import TargetShare
from mtee.testing.test_environment import TEST_ENVIRONMENT as TE
from si_test_idcevo import LIFECYCLES_PATH
from si_test_idcevo.si_test_helpers.android_testing.test_base import TestBase
from si_test_idcevo.si_test_helpers.csv_handlers import CSVHandler
from si_test_idcevo.si_test_helpers.dltlyse_utils import DltlyseCustomHandler, _collect_dltlyse_plugin_package_dirs

# Config parser reading data from config file.
config = configparser.ConfigParser()
config.read(Path(__file__).parent.resolve() / "features_config.ini")
logger = logging.getLogger(__name__)


class DLTlysePluginPostTest(object):

    __test__ = True

    target = TargetShare().target

    @classmethod
    def setup_class(cls):
        cls.test = TestBase.get_instance()
        cls.test.setup_base_class(skip_setup_apinext=True)

        cls.plugins_list = ["DLTMsgInterestPersoPlugin"]
        cls.package_names = ["dltlyse_plugins_gen22"]
        cls.package_path = str(Path(Path(__file__).parent.absolute().parent, "dltlyse_plugins"))
        logger.info("Package path: %s", cls.package_path)

        cls.result_dir = cls.target.options.result_dir
        cls.dlt_trace_file = os.path.join(cls.result_dir, "idcevo_full_trace.dlt")
        cls.timeout = 60

    def create_temporary_copy(self, path, temp_name="idcevo.dlt"):
        temp_dir = tempfile.gettempdir()
        temp_path = os.path.join(temp_dir, temp_name)
        shutil.copy2(path, temp_path)
        return [temp_path]

    @skipIf(not target.has_capability(TE.test_bench.rack), "Skip the test for rack")
    def test_001_custom_dltlyse_plugin(self):
        """Test custom DLTlyse plugin

        This test only allows to run dltlyse plugins like DLTMsgInterestPluginCustom
        """

        plugins_dirs = _collect_dltlyse_plugin_package_dirs(
            plugins_dirs=[self.package_path],
            package_names=self.package_names,
        )
        plugins_dirs.append(self.package_path)

        temp_dlt_trace_file = self.create_temporary_copy(self.dlt_trace_file)

        dltlyse_proc = DltlyseCustomHandler(
            plugins=self.plugins_list,
            plugins_dir=plugins_dirs,
            dlt_trace_file=temp_dlt_trace_file,
            work_dir=self.result_dir,
        )

        try:
            output_process = dltlyse_proc.start_dltlyse()
            if re.search("Errors found on lifecycle", output_process, re.IGNORECASE):
                raise AssertionError("Errors found, please check the dltlyse plugins for details")
        finally:
            dltlyse_proc.stop_dltlyse()
            return

    def test_002_missing_account(self):
        """Missing account ID in DLT messages

        This test only allows to run dltlyse plugins like DLTMsgInterestPluginCustom
        """
        apid_filter = "ALD"
        ctid_filter = "LCAT"
        error_msg = "MISSING_ACCOUNT_ID"

        error_detected = False
        error_message_files = []
        pattern = re.compile(r"IAccountDataServiceImpl")

        # Only checking messages after creating BMW user account
        tests_started = False
        test_name = re.compile(r"TEARDOWN_START test_tearDown.*\[SIT_Automated\] Add a new user using BMWID.*")

        lifecyle_full_path = os.path.join(self.test.mtee_target.options.result_dir, LIFECYCLES_PATH)
        csv_handler = CSVHandler("dlt_msgs_interest_perso.csv")
        # gets list containing all csv files path
        self.files_path = csv_handler.get_csv_files_path(lifecyle_full_path)

        number_lifecycles = len(self.files_path)
        assert number_lifecycles > 0, "An error occurred while running the test: No csv found."

        for csv_file in self.files_path:
            with open(csv_file) as f:
                reader = csv.DictReader(f)

                for row in reader:
                    if (
                        (row["apid"] == apid_filter)
                        and (row["ctid"] == ctid_filter)
                        and pattern.search(row["payload"])
                    ):
                        if error_msg in row["payload"] and tests_started:
                            error_message_files.append(csv_file)
                            error_detected = True
                            break
                    elif row["apid"] == "MTEE" and row["ctid"] == "MTEE" and test_name.search(row["payload"]):
                        tests_started = True

        assert (
            not error_detected
        ), f"The error message '{error_msg}' was found in the CSV files - '{error_message_files}'"

    def test_003_check_perso_app(self):
        """Check version of the perso app"""

        with open(os.path.join(self.test.mtee_target.options.result_dir, "perso_app_version.txt"), "r") as f:
            perso_app_version = f.read()
            print(f"Perso app version: {perso_app_version}")  # noqa: T201
        assert (
            "snapshot" not in perso_app_version.lower()
        ), f"Perso app version should not be a dirty version: {perso_app_version}"
