...
class TestLauncherPADI(TestCase):
    ...
    # The element video is the place holder for the CE area
    ...
    def __toggle_hdmi_and_ce(self, panel, toggle_upto=10):
        """
        open PADI display and toggle 10 times HDMI and Amazon fire TV and close the display


         Args:
             panel - class, LEFT or RIGHT side panel class object
             toggle_upto - max time toggle
         Note: first time ignoring NoSuchElementException because status can display be changed second time
              throwing exception
        """
        panel_settings = panel.click_side_panel_menu_button(self.test, panel, panel.SIDE_PANEL_INPUT_MENU_BUTTON_ID)
        for i in range(toggle_upto):
            try:
                # locating HDMI ID and clicking on it
                hdmi_button = panel_settings.find_element(*panel.SIDE_INPUT_HDMI_BUTTON_ID)
                hdmi_button.click()
                time.sleep(1)
                Launcher.stop_hdmi_animation()
                time.sleep(1)
                # locating CE ID and clicking on it
                ce_button = panel_settings.find_element(*panel.SIDE_INPUT_EXTBOARD_BUTTON_ID)
                ce_button.click()
                time.sleep(1)
            except (TimeoutException, StaleElementReferenceException):
                panel_settings = panel.click_side_panel_menu_button(
                    self.test, panel, panel.SIDE_PANEL_INPUT_MENU_BUTTON_ID
                )
        else:
            # Returning back to actual screen pos
            panel.click_on_child_element(
                self.test, panel_settings, panel.SIDE_INPUT_EXTBOARD_BUTTON_ID, panel.SIDE_INPUT_HDMI_BUTTON_ID
            )
    ...
    def test_004_check_default_ratio(self):
        ...
        *Steps*
        ...
        for ratio, ratio_elem in LeftPanel.ce_area_ratios_dict.items():
            ...
            # Tap on the middle of the screen (in CE_area panel)
            ce_area_elem.click()
            time.sleep(1)
    ...
    @utils.gather_info_on_fail
    def test_006_toggle_hdmi_and_ce(self):
        """
        Input window toggle between "HDMI" & "CE"

        *Background information*
        The aim of  this test is to verify that Basic input settings are be stable and functional all the time.

        *Steps*
        1.  Ensure PADI is up.
        2.  Go to input settings on Left Side of the Panel for PADI.
        3.  Try to select input as "HDMI" and the switch back to "CE"
        4.  Go to Step 2 and follow the steps from 2-3 for input settings of the Right Panel.
        5.  Try the Toggle  "HDMI" to "CE" 10 times.
        """
        self.__toggle_hdmi_and_ce(LeftPanel)
        self.__toggle_hdmi_and_ce(RightPanel)
