...
class TestLanguageSettings:
    ...
    def setup_class(cls):
        ...
        if not Launcher.validate_launcher():
            ...
            cls.test.setup_base_class()
        Settings.language_change(cls.languages_id_dict["de"])
        utils.start_recording(cls.test)

    @classmethod
    def teardown_class(cls):
        Launcher.go_to_home()
        Settings.language_change(cls.languages_id_dict["en_GB"])
    ...
    def __check_available_languages_list(self, language_button, language_dict):
        ...
        if language_button.find_element(*Settings.RADIO_BUTTON_ITEM).get_attribute("checked") != "true":
            language_dlt = re.compile(r"LANGUAGE_JAVA_SERVICE\[\d+\]: Set vehicle's language to\s*:? (\w+[- ]\w+|\w+)")
            ...
            logger.debug(f"Current language name is {language_name}")
    ...
    def test_000_check_and_click_lngbtn(self):
        ...
        *steps*
        ...
        """
        Launcher.go_to_home()
        Settings.launch_settings_activity(validate_activity=False)
