...
class TestKinematicPop(TestCase):
    ...
    def check_popup_text(self, popup_text, screenshot):
        ...
        param: popup_text - Expected popup string
        param: screenshot - Screenshot file captured when popup appears
        """
        # For the PADi HDMI variant, the target is the same as China and the image is the same as RoW.
        # Only coding is different.
        # So the language of the target, despite being a padi_china, is English
        ...
        if popup_content or popup_content_pu:
            assert (
                popup_text in search_element.text
            ), f"Expected popup text is: {popup_text} But found: {search_element.text}"
        else:
            test_result, message = check_screendump(screenshot, popup_text, lang="eng")
            if not test_result:
    ...
    def test_003_padiblockedbyfrontseatposition(self):
        ...
        *Steps*
        ...
        self.vcar_manager.send("ControlPaDI.statusPaDi.statusPadiPopUpDirection = 3")
        time.sleep(0.5)
        popup_text = "Please move the front seats forwards to ensure that the Theatre Screen is not blocked."
        screenshot_path = capture_screenshot(
            test=self.test,
    ...
    def test_004_padicollisionavoidancebecauseseatmoving(self):
        ...
        *Steps*
        ...
        self.vcar_manager.send("ControlPaDI.statusPaDi.statusPadiPopUpDirection = 3")
        time.sleep(0.5)
        popup_text = "Front seat adjustment active. Adjusting position of the Theatre Screen"
        screenshot_path = capture_screenshot(
            test=self.test,
    ...
    def test_006_padioveruse(self):
        ...
        *Steps*
        ...
        self.vcar_manager.send("ControlPaDI.statusPaDi.statusPadiPopUpDirection = 3")
        time.sleep(0.5)
        popup_text = "Function temporarily deactivated due to overloading."
        screenshot_path = capture_screenshot(
            test=self.test,
